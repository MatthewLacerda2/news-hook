...
@pytest.mark.asyncio
async def test_patch_alert_wrong_user(client, valid_user_with_credits, mock_google_verify, sample_llm_models, test_db, mock_llm_validation):
    """Test attempting to patch an alert belonging to another user"""
    user_data = valid_user_with_credits
    api_key = user_data["api_key"]
    ...
    # Try to patch alert with second user's API key
    response = await client.patch(
        f"/api/v1/alerts/{alert_id}",
        headers={"X-API-Key": api_key2},
        json={"http_url": "https://unauthorized-update.example.com"}
    )
    
    assert response.status_code == 404
    assert "Not found" in response.json()["detail"]

#tests for patch alert